...
function writeScalar(state: DumperState, string: string, level: number, iskey: boolean) {
    state.dump = (() => {
        if (string.length === 0) {
            return "''";
        }
...
function writeBlockSequence(state: DumperState, level: number, object: any, compact = false) {
    let _result = '';
    const _tag = state.tag;
...
function writeBlockMapping(state: DumperState, level: number, object: object, compact = false) {
    const _tag = state.tag,
        objectKeyList = Object.keys(object);
    let _result = '';
...
function inspectNode(object: object, objects: object[], duplicatesIndexes: number[]) {
